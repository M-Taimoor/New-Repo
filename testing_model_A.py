...
class MobileBankingApp:
    ...
    def show_transactions(self, user):
        ...
        """
        try:
            self.cursor.execute('SELECT * FROM authentication_logs WHERE user_id = ?', (user.id,))
            transactions = self.cursor.fetchall()
            print("Transaction History:")
            for transaction in transactions:
                print(f"Timestamp: {transaction[2]}, Success: {transaction[3]}, OTP Used: {transaction[4]}")
        except sqlite3.Error as e:
